...
export default function ProductClient({
  ...
  const [discountTimeLeft, setDiscountTimeLeft] =
    useState<DiscountTimeLeft | null>(() =>
      getDiscountTimeLeft(product.discountEndsAt),
    );
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setDiscountTimeLeft(getDiscountTimeLeft(product.discountEndsAt));
    }, 1000);
