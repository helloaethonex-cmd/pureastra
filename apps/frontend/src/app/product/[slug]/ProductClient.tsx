...
function BuyNowAddressForm({
  ...
  const field = (key: keyof typeof EMPTY_FORM, label: string, type = "text", half = false) => (
...
function BuyNowPanel({
  ...
  const { data: addresses, isLoading: addrLoading } = useMyAddresses(Boolean(user));
      ...
      <div className="relative z-10 w-full sm:w-[480px] max-h-screen sm:max-h-[90vh] bg-[#FDF8F1] sm:rounded-2xl shadow-2xl overflow-y-auto flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {step === "address" && (
            <div>
              {addrLoading ? (
                <p className="text-sm text-[#7B6A58] py-6 text-center">Loading addresses...</p>
              ) : (
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                  {(addresses ?? []).map((addr) => (
                    ...
                    >
                      <p className="font-semibold text-sm text-[#3d2b1a]">{addr.fullName}</p>
              ...
              {showForm && (
                <BuyNowAddressForm
                  onSaved={(addr) => { setSelectedId(addr.id); setShowForm(false); }}
                  onCancel={() => setShowForm(false)}
                />
              )}

              {previewError && (
                <p className="mt-2 text-xs text-red-500 text-center">{previewError}</p>
              )}
          ...
          {step === "preview" && preview && (
            <div>
              <div className="bg-[#F5F0E6] border border-[#D6C9B6] rounded-xl p-4 mb-5">
                ...
                <p className="text-xs text-[#9a7a65] mt-1">Inclusive of all taxes &amp; FREE shipping</p>
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {buyNow.isPending ? "Processing..." : `Pay ₹${Number(preview.grandTotal).toFixed(2)}`}
                </button>
              </div>
            </div>
          )}
...
export default function ProductClient({ product }: { product: Product }) {
  ...
  const reviewsQuery = useProductReviews(product.id, { page: 1, limit: 10, sortBy: "newest" });
  const reviewSummaryQuery = useProductReviewSummary(product.id);
  const reviewMetricsQuery = useProductReviewMetrics(product.id);
  const reviewEligibilityQuery = useReviewEligibility(product.id, Boolean(user));
    ...
    <section className="bg-[#FAF3E2]">
      ...
      <div className="px-4 md:px-12 py-6 md:py-10 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
        ...
        <div>
          ...
          >
            <motion.div whileHover={{ scale: 1.2 }} transition={{ duration: 0.4 }} className="w-full h-full">
              <Image
            ...
            {tab === "reviews" && (
              ...
              >
                <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="text-[18px] md:text-[22px] text-[#5e2b16] font-bold">
                      {reviewSummaryQuery.isLoading ? "..." : (reviewSummary?.avgRating ?? 0).toFixed(1)}
                    </span>
                    ...
                    <span className="bg-green-100 text-green-700 px-2 py-[2px] rounded text-xs md:text-sm">
                      {(reviewSummary?.avgRating ?? 0) >= 4 ? "Very Good" : "Good"}
                    </span>
                  </div>
                ...
                <p className="text-[12px] md:text-[14px] text-[#8B5E3C] mb-4">
                  based on {reviewSummary?.totalReviews ?? 0} ratings by verified buyers
                </p>
                    ...
                    {reviewSummary?.metrics.map((metric) => (
                      <div key={metric.metricId}>
                        ...
                        <div className="w-full bg-white/60 rounded-full h-2">
                          <div className="bg-[#819744] h-2 rounded-full" style={{ width: `${metric.average}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {showAddReview && reviewEligibility?.canReview && (
                  <div className="mb-5 bg-[#F5F0E6] p-4 rounded-xl border border-[#E6DCCB] space-y-3">
                    <p className="text-sm font-semibold text-[#5E2B16]">Write your review</p>
                      ...
                      <div className="flex items-center gap-2 text-[#819744]">
                        {[1, 2, 3, 4, 5].map((star) => (
                          ...
                          >
                            <FontAwesomeIcon
                              icon={faStar}
                              className={star <= reviewRating ? "opacity-100" : "opacity-35"}
                            />
                          </button>
                        ))}
                      </div>
                    ...
                    {reviewMetrics.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-xs text-[#8B5E3C]">Rate product metrics</p>
                        {reviewMetrics.map((metric: ReviewMetric) => {
                          ...
                          const percentage = ((value - metric.minValue) / range) * 100;
                          return (
                            <div key={metric.id}>
                              <div className="flex items-center justify-between text-xs text-[#5E2B16] mb-1">
                                ...
                                <span>{value}{metric.unit === "PERCENT" ? "%" : ""}</span>
                              </div>
                              <input
                                type="range"
                                min={metric.minValue}
                ...
                <div className="flex gap-4 overflow-x-auto scroll-smooth snap-x snap-mandatory pb-2">
                  ...
                  {!reviewsQuery.isLoading && reviewList.length === 0 && (
                    <p className="text-sm text-[#8B5E3C]">No reviews yet. Be the first to review after purchase.</p>
                  )}
                    ...
                    >
                          ...
                          {review.metrics.map((metric) => (
                            <div key={`${review.id}-${metric.metricId}`}>
                              <div className="flex justify-between items-center text-[#5E2B16] mb-1">
                                ...
                                <span>{metric.value}{metric.unit === "PERCENT" ? "%" : ""}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="flex justify-between items-center text-[11px] md:text-[12px]">
                        <div className="flex items-center gap-2 text-[#819744] font-semibold">
                          {review.isVerifiedPurchase && <FontAwesomeIcon icon={faCheckCircle} />}
                          {review.isVerifiedPurchase ? "Verified" : "Customer"}
                        </div>

                        <div className="flex text-[#819744]">
                          {[...Array(review.rating)].map((_, i) => (
                            <FontAwesomeIcon key={`${review.id}-rate-${i}`} icon={faStar} />
                          ))}
                        </div>
                      </div>
