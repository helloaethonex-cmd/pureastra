...
function HighlightsSection({
  ...
  const items =
    (content?.items as { text: string }[] | undefined) ?? bullets.map((b) => ({ text: b.text }));
...
function SuitableForSection({
  ...
  const fields: { label: string; value: string }[] = content?.fields ?? [
    { label: "Skin Type", value: "All skin types, including sensitive, dry, oily, and combination skin" },
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 text-[#5E2B16] text-[16px]">
        {steps.map((step, i) => (
          <div key={i} className="flex gap-3 items-start font-['Roboto_Flex'] text-[14px] text-[#5C6936]">
            <FontAwesomeIcon
...
function IngredientsSection({
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 bg-white rounded-xl shadow-md p-6">
      <h3 className="text-[#819744] font-semibold text-[22px] mb-4">Ingredients</h3>
      {text && <p className="text-sm text-[#5f5f5f] leading-6">{text}</p>}
      {list.length > 0 && (
...
export default function ProductClient({ product }: { product: Product }) {
  ...
  const relatedProducts = (relatedData?.data ?? []).filter((p) => p.id !== product.id).slice(0, 3);
  ...
  return (
    <section className="bg-[#FAF3E2]">
      ...
      <div className="px-6 md:px-12 py-10 grid grid-cols-2 gap-10 max-md:grid-cols-1">
        ...
        {/* RIGHT — Details */}
        <div className="pt-2">
          <h2 className="text-[30px] font-bold text-black mb-2">{product.name}</h2>
          ...
          >
            {product.description ?? "A thoughtfully crafted product by Pureastra."}
          </motion.p>

          {/* Variants */}
          {product.variants.length > 0 && (
            <div className="mb-5 flex items-center gap-4">
              <p className="font-['Roboto_Flex'] font-semibold text-[20px] text-black">Size :</p>
          ...
          {/* Suited for — from SUITABLE_FOR section shortcut field */}
          {(() => {
            const skinType = (suitableSection?.content as any)?.skinType as string | undefined;
            return skinType ? (
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 px-6 items-center">
          ...
          >
            <div className="relative w-[520px] h-[420px]">
              <Image src="/img/aditional.png" alt="illustration" fill className="object-contain" />
            </div>
          </motion.div>
        </div>
      </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {relatedProducts.map((item, i) => {
              const img =
                ...
                "/img/facewash.webp";
              const price = item.variants.reduce(
      ...
      <div className="bg-[#F5F0E6] py-16 px-6">
        ...
        <IngredientsSection section={ingredientsSection} />

        {/* ADDITIONAL INFO */}
        <div className="max-w-4xl mx-auto mt-16 bg-white rounded-xl shadow-md p-6">
          <h3 className="text-[#819744] font-semibold text-[22px] mb-6">Additional Information</h3>
